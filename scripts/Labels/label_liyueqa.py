...
# Load cleaned data
df = pd.read_csv("data/processed/cleaned_liyueqa_messages.csv")
...
# Save to text file
with open("liyueqa_labeling_template.conll", "w", encoding="utf-8") as f:
    for tokens in label_df['tokens']:
        for token in eval(tokens):
            f.write(f"{token} O\n")  # default label 'O', ready to be updated
        f.write("\n")  # separate messages

print("✅ Saved liyueqa_labeling template to labeling_template.conll")
